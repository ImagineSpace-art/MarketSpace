...
type SettingsPageProps = {
    ...
    setGlobalRenewalFrequency?: React.Dispatch<React.SetStateAction<'daily' | 'weekly' | 'biweekly' | 'monthly'>>
    globalRenewalDay?: 'Monday' | 'Tuesday' | 'Wednesday' | 'Thursday' | 'Friday' | 'Saturday' | 'Sunday'
    setGlobalRenewalDay?: React.Dispatch<React.SetStateAction<'Monday' | 'Tuesday' | 'Wednesday' | 'Thursday' | 'Friday' | 'Saturday' | 'Sunday'>>
    enableGlobalRenewal?: boolean
    setEnableGlobalRenewal?: React.Dispatch<React.SetStateAction<boolean>>
}
                    ...
                    <div className="setting-box" style={{ background: 'var(--panel)', padding: '20px', borderRadius: '12px', border: '1px solid var(--border)' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '10px' }}>
                            ...
                            <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', fontWeight: 600, fontSize: '0.9rem', background: 'var(--surface)', padding: '8px 14px', borderRadius: '8px', border: '1px solid var(--border)' }}>
                                <input
                                    type="checkbox"
                                    checked={enableGlobalRenewal}
                                    onChange={(e) => setEnableGlobalRenewal && setEnableGlobalRenewal(e.target.checked)}
                                    style={{ width: '18px', height: '18px', cursor: 'pointer' }}
                                />
                                <span>Use global renewal time</span>
                            </label>
                        </div>

                        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', opacity: enableGlobalRenewal ? 1 : 0.45, pointerEvents: enableGlobalRenewal ? 'auto' : 'none' }}>
                            {/* Frequency Dropdown */}
                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
                                <label style={{ fontSize: '0.88rem', fontWeight: 600, minWidth: '160px' }}>Renewal Interval:</label>
                                <select
                                    disabled={!enableGlobalRenewal}
                                    value={globalRenewalFrequency}
                                    onChange={(e) => setGlobalRenewalFrequency && setGlobalRenewalFrequency(e.target.value as any)}
                                    style={{ padding: '8px 12px', borderRadius: '8px', border: '1px solid var(--border)', background: 'var(--surface)', color: 'var(--text)', fontWeight: 600 }}
                                >
                            ...
                            {/* Preferred Day of Week Selection */}
                            <div>
                                <label style={{ fontSize: '0.88rem', fontWeight: 600, display: 'block', marginBottom: '8px' }}>
                                    Preferred Day of Week to Renew:
                                </label>
                                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                                    {(['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'] as const).map((day) => {
                                        const isSelected = globalRenewalDay === day
                                        return (
                                            <button
                                                key={day}
                                                type="button"
                                                disabled={!enableGlobalRenewal}
                                                onClick={() => setGlobalRenewalDay && setGlobalRenewalDay(day)}
                                                style={{
                                                    padding: '8px 14px',
                                                    borderRadius: '8px',
                                                    border: isSelected ? '2px solid #2563eb' : '1px solid var(--border)',
                                                    background: isSelected ? 'rgba(37, 99, 235, 0.12)' : 'var(--surface)',
                                                    color: isSelected ? '#2563eb' : 'var(--text)',
                                                    fontWeight: isSelected ? 700 : 500,
                                                    cursor: enableGlobalRenewal ? 'pointer' : 'not-allowed',
                                                    fontSize: '0.86rem'
                                                }}
                                            >
                                                {day}
